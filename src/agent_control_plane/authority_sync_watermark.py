...
def decode_authority_sync_watermark(payload: bytes | str) -> AuthoritySyncWatermark:
    if isinstance(payload, bytes):
        try:
            ...
            raise AuthorityValidationError("watermark payload must be valid UTF-8") from exc
    elif isinstance(payload, str):
        text = payload
    else:
        raise AuthorityValidationError("watermark payload must be bytes or str")
    try:
        data = json.loads(text)
    except json.JSONDecodeError as exc:
        raise AuthorityValidationError("watermark payload must be valid JSON") from exc
    if not isinstance(data, Mapping):
        raise AuthorityValidationError("watermark payload must decode to an object")
    expected = {
        "watermark_id",
        "issuer_id",
        "target_sender_id",
    ...
    except (TypeError, ValueError, AuthorityValidationError):
        raise
...
class AuthoritySyncWatermarkRegistry:
    ...
    def apply(self, watermark: AuthoritySyncWatermark) -> None:
        if not isinstance(watermark, AuthoritySyncWatermark):
            raise AuthorityValidationError("watermark must be AuthoritySyncWatermark")
